...
const createOrder = async (req, res) => {
  ...
  try {
    ...
    const cartResult = await request
      ...
      .query(
        "SELECT CartID, TotalPrice FROM Carts WHERE UserID = @userID AND Status = 'active'"
      );

    if (
      cartResult.recordset.length === 0 ||
      cartResult.recordset[0].TotalPrice <= 0
    ) {
    ...
    await request
      ...
      .query(
        "INSERT INTO Payments (OrderID, Amount, PaymentStatus) VALUES (@orderID, @amount, @paymentStatus)"
      );

    // Mark the cart as processed
    await request.query(
      "UPDATE Carts SET Status = 'processed' WHERE CartID = @cartID"
    );
...
const getAllOrders = async (req, res) => {
  ...
  try {
    ...
    const ordersResult = await request.input("userID", sql.Int, userID).query(`
                SELECT 
...
const getOrderDetails = async (req, res) => {
  ...
  if (!orderID) {
    console.warn(
      "getOrderDetails: missing orderID, req.params=",
      req.params,
      "req.query=",
      req.query
    );
  ...
  try {
    ...
    if (orderDetails.CartID) {
      ...
      const cartItemsResult = await cartRequest.input(
        "cartID",
        sql.Int,
        orderDetails.CartID
      ).query(`
...
const updatePaymentStatus = async (req, res) => {
  ...
  try {
    ...
    const orderCheck = await request
      ...
      .input("userID", sql.Int, userID)
      .query(
        "SELECT OrderID FROM Orders WHERE OrderID = @orderID AND UserID = @userID"
      );
    ...
    await request
      ...
      .query(
        "UPDATE Payments SET PaymentStatus = @paymentStatus WHERE OrderID = @orderID"
      );
